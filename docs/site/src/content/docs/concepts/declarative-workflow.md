...
Safety red lines enforced by the skill: direct/manual cluster mutations are forbidden, blanket staging (`git add .`) is refused, and `submit_suggestion.py` hard-blocks pushes to the protected branches `main`, `master`, and `production`. The push is `--force-with-lease`, so re-submitting after review feedback updates the existing PR branch but will not overwrite one somebody else has moved. The credential proxy refuses the same three branches underneath the skill, along with `gh pr merge` and the rest of the non-proposal GitHub surface — see [security and IAM](/kube-agents/reference/security-and-iam/#a-third-wall-the-credential-proxy-allows-only-branch-and-pull-request). Bypassing the skill by running `git` directly therefore does not bypass the branch check.
