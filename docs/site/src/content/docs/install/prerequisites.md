...
- **`ssh-keygen`** — only for Method 2, which mints the shell sandbox SSH keypair by hand in [INSTALL.md Step 2](https://github.com/gke-labs/kube-agents/blob/main/INSTALL.md#step-2-create-api-key--access-secrets); the installer, Terraform and `upgrade.sh` mint it for the other methods. Ships with the OpenSSH client.
