...
- **CLI commands** — only `gcloud`, `kubectl`, `gh`, and `git` are accepted. The proxy rejects known credential-disclosure, credential-replacement, and self-modification operations; interactive TTY programs, unbounded streaming, sandbox-only file paths, and background processes fail closed. On top of those pattern rules, two argument-vector gates decide what `kubectl`, `gh` and `git push` may do at all: the [read-only `kubectl` gate](/kube-agents/reference/security-and-iam/#a-second-wall-the-credential-proxy-refuses-mutating-kubectl) and the [branch-and-pull-request gate](/kube-agents/reference/security-and-iam/#a-third-wall-the-credential-proxy-allows-only-branch-and-pull-request).
