...
| **The GitOps path**           | Changes to your infrastructure-as-code repository                                                                                                                  | Yes — by opening a pull request a human must review and merge. Enforced by the credential proxy, which allows the agent's GitHub token to push a branch and open a pull request and refuses it the merge (see [below](#a-third-wall-the-credential-proxy-allows-only-branch-and-pull-request)). |
...
### A third wall: the credential proxy allows only branch-and-pull-request

"The agent proposes; a human merges" is the third row of the table above, and until recently it described an intention rather than a boundary. The agent holds a short-lived, repository-scoped GitHub App installation token minted by [Minty](/kube-agents/deploy/token-minter/), and an installation token with `contents: write` and `pull_requests: write` can merge the pull request it just opened, approve its own review, force-push over `main` and delete a branch. Nothing on the GitHub side distinguishes those from opening the pull request.

The same sidecar that gates `kubectl` gates `gh` and `git`, and it does so with two checks.

**For `gh`, an allowlist of commands.** `GH_ALLOWED_COMMANDS` in `credential_proxy.py` is the list, and it is the only authoritative statement of it; in outline, the reads (`pr` and `issue` `list`/`view`/`status`, `pr diff`/`checks`, `repo view`, `label`/`run`/`workflow`/`release` listing and viewing, `auth status`, `search`, `status`, `version`) plus the proposal path itself (`pr create`/`edit`/`comment`/`close`/`reopen`/`ready`, the first five of those for `issue`, which has no `ready`, and `label create`, which two skills call to create the status labels they then apply). An unrecognised command is refused, so a `gh` release that adds a new write command is refused by a proxy that has never heard of it — and so is an alias, including gh's built-in `co`, because the match is on the literal argv rather than on what `gh` would expand it to. What that leaves out is the point:

- **`pr merge`** — a proposal the proposer can accept is not a proposal.
- **`pr review`** — self-approval is merging with extra steps on any repository whose branch protection counts approvals.
- **`pr checkout`** — a working-tree write that would go around the GitOps workspace lease, which only knows how to read `git` argv.
- **`repo` beyond `view`, and `secret`, `variable`, `ssh-key`, `gpg-key`, `extension`, `alias`** — repository administration and tool or credential self-modification.
- **`workflow run`/`enable`/`disable` and `run rerun`/`cancel`** — dispatching CI is arbitrary code execution wearing a different hat. This one is defence in depth rather than a live hole: the App this repository provisions is scoped to `contents`, `pull_requests` and `issues`, so the token cannot dispatch a workflow (that needs `actions: write`) or set a secret (`secrets: write`) today. The refusal is for the install that widens the App without revisiting the proxy.
- **`gh api` with anything but a `GET`.** `gh api` is allowed as a read because skills use it to fetch inline review comments, but a `--method` other than GET or HEAD is refused, and so is a bare field flag — `-f`/`-F`/`--input` silently switch gh's default method to POST, so the argv that files an issue is one flag away from the argv that reads one. `gh api graphql` is refused by name, whatever the method: GraphQL carries its verb inside `query=`, where no rule that reads the argument vector can see it, so `--method GET -f query=mutation{…}` would otherwise pass as a query parameter.

**For `git push`, a check on the destination.** `push` is the only remote-write verb git has, so there is no verb to allow or deny; what the proxy checks is where the refspec lands. It refuses a push to `main`, `master` or `production` (`GIT_PROTECTED_BRANCHES` — deliberately the same three names the `submit-suggestion` and `fleet-audit` helpers have always refused, so that both layers answer the same question the same way); a deletion, whether written `--delete` or `:branch`; `--all` and `--mirror`, which push refs the command line does not name; and a bare `git push` with no refspec, which lands wherever `push.default` and the current branch send it and so cannot be checked at all — name the branch. Force is deliberately **not** refused: a pull request that comes back for another round of review has to update the branch it already points at, which is why `submit-suggestion` pushes `--force-with-lease`. What makes a force dangerous is where it lands, and that is what is checked. Nor is `--dry-run` (`-n`), which is exempt from all four refusals above — it writes nothing, and a skill that wants to find out what a push would do should not have to guess.

Unlike the `kubectl` gate, **this one ships enforcing.** A survey of every `gh` and `git push` call site in the tree found none that it refuses: the shipped skills only ever list, view, create, edit, comment, close, `label create` and `auth status`, and every push names its branch. A gate that refuses nothing anybody does is a gate that can start refusing. Set `CREDENTIAL_PROXY_FORGE_MODE` to `warn` (log and run anyway) or `off` on a deployment that disagrees, and confirm what the proxy resolved from its startup line, `forge branch-and-pr gate mode=… protected=…`.

A refusal is an HTTP 403 carrying `"code": "SECURITY_POLICY_BLOCKED"` and either `"rule": "github.write-path"` or `"rule": "git.push-target"`. The policy document also accepts `allowedCommands` and `protectedBranches` under a `forge` key, with the same two caveats as the `kubernetes` key above: each **replaces** its default rather than extending it, and neither is reachable on an operator-managed install.

Two things this does not do. It does not bound what the token could do if it leaked — it constrains the agent's command line, not GitHub, so the real ceiling is still the App installation's permissions, which is the argument for scoping those down as well. And it says nothing about _content_: an allowed `gh pr create` can open a pull request containing anything the agent staged, which is why blanket staging is refused separately, one layer down in the skills.
...
Both paths share the same guardrails, in two layers. The skills refuse blanket staging (`git add .` / `git add -A`) and refuse to target `main`, `master` or `production`. The credential proxy refuses the same three branch targets underneath them, along with the merge itself — see [branch-and-pull-request](#a-third-wall-the-credential-proxy-allows-only-branch-and-pull-request). The distinction matters: the skill-layer check is bypassed by running `git` directly instead of the helper, and the proxy-layer one is not.
